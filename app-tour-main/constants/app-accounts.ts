...
// ─────────────────────────────────────────────────────────────
const AVATAR_COLORS = ['#4f7cff','#10b981','#f59e0b','#8b5cf6','#ef4444','#06b6d4','#ec4899'];

const SEED_ACCOUNTS: AppAccount[] = [
  {
    id:'acc-guest-1', email:'guest1@gmail.com', password:'123456',
    name:'Nguyễn An', phone:'0901 234 567',
    roles:['guest'], activeRole:'guest', status:'active',
    createdAt:'2025-01-10T08:00:00Z', avatarColor:'#4f7cff',
  },
  {
    id:'acc-admin-1', email:'admin1@gmail.com', password:'123456',
    name:'Trần Quang Admin', phone:'0909 123 456',
    roles:['admin'], activeRole:'admin', status:'active',
    createdAt:'2025-01-01T00:00:00Z', avatarColor:'#ef4444',
  },
  {
    id:'acc-guide-1', email:'guide1@gmail.com', password:'123456',
    name:'Nguyễn Văn Hùng', phone:'0912 345 678',
    roles:['guide','guest'], activeRole:'guide', status:'active',
    createdAt:'2025-01-05T08:00:00Z', avatarColor:'#10b981',
    guideStatus:'active', guideId:'g1',
  },
  {
    id:'acc-guide-2', email:'guide2@gmail.com', password:'123456',
    name:'Trần Thị Lan', phone:'0918 765 432',
    roles:['guide','guest'], activeRole:'guide', status:'active',
    createdAt:'2025-01-07T08:00:00Z', avatarColor:'#8b5cf6',
    guideStatus:'active', guideId:'g2',
  },
  // Guest có cả 2 role (đã được duyệt làm HDV)
  // Guest có cả 2 role (đã được duyệt làm HDV)
  {
    id:'acc-dual-1', email:'dual1@gmail.com', password:'123456',
    name:'Lê Văn Phong', phone:'0933 111 222',
    roles:['guest','guide'], activeRole:'guest', status:'active',
    createdAt:'2025-02-01T08:00:00Z', avatarColor:'#06b6d4',
    guideStatus:'paused', guideId:'g4',
  },
  // Staff CSKH
  {
    id:'acc-staff-1', email:'staff1@gmail.com', password:'123456',
    name:'Lê Thị CSKH', phone:'0911 000 111',
    roles:['staff'], activeRole:'staff', status:'active',
    createdAt:'2025-01-02T00:00:00Z', avatarColor:'#f59e0b',
  },
];
// ─────────────────────────────────────────────────────────────
// Keys
...
export async function registerAccount(
  ...
  const newAcc: AppAccount = {
    id: `acc-${Date.now()}`, email: email.toLowerCase(), password,
    name, phone, roles: ['guest'], activeRole: 'guest',
    status: 'active', createdAt: new Date().toISOString(), avatarColor: color,
  };
...
export async function loginAccount(
  ...
  if (acc.status === 'suspended') return { ok: false, error: 'Tài khoản đã bị tạm khóa.' };
...
export async function switchRole(
  accountId: string, newRole: UserRole
  ...
  const acc = list[idx];
  if (!acc.roles.includes(newRole)) return { ok: false, error: `Tài khoản không có quyền ${newRole}.` };
...
export async function setGuideStatus(
  ...
  // Sync vào @app_guides
  if (list[idx].guideId) {
    await syncGuideStatusToPublic(list[idx].guideId!, status, pauseFromBookings);
  }
  // Update session
  const user = accountToCurrentUser(list[idx]);
  await saveCurrentUser(user);
...
export async function submitGuideRequest(
  ...
  bio: string, skills: string, languages: string, experience: string
): Promise<void> {
  ...
  const existing = list.find(r => r.accountId === accountId && r.status === 'pending');
  if (existing) return;
  const req: GuideRequest = {
    id: `req-${Date.now()}`, accountId, name, email, phone,
    bio, skills, languages, experience,
    status: 'pending', createdAt: new Date().toISOString(),
  };
  await AsyncStorage.setItem(KEY_REQUESTS, JSON.stringify([req, ...list])).catch(() => {});
}
...
export async function approveGuideRequest(requestId: string): Promise<void> {
  ...
  const updatedReqs = reqs.map(r => r.id === requestId ? { ...r, status: 'approved' as const } : r);
  ...
  const upgraded: AppAccount = {
    ...list[idx],
    roles: list[idx].roles.includes('guide') ? list[idx].roles : [...list[idx].roles, 'guide'],
    guideStatus: 'active',
    guideId,
  };
  list[idx] = upgraded;
...
export async function rejectGuideRequest(requestId: string, note?: string): Promise<void> {
  const raw = await AsyncStorage.getItem(KEY_REQUESTS).catch(() => null);
  const reqs: GuideRequest[] = raw ? JSON.parse(raw) : [];
  const updated = reqs.map(r => r.id === requestId ? { ...r, status: 'rejected' as const, note } : r);
...
export async function syncGuideProfileToPublic(
  guideId: string,
  profile: {
    name: string; location: string; experience: string;
    skills: string; bio: string; phone: string; email: string;
    rating?: number; tours?: number; match?: number;
  }
): Promise<void> {
  try {
    ...
    const skillArr = profile.skills.split(',').map((s: string) => s.trim()).filter(Boolean);
    if (idx >= 0) {
      list[idx] = {
        ...list[idx],
        name:       profile.name,
      ...
      list.unshift({
        id: guideId, name: profile.name, location: profile.location,
        experience: profile.experience, skills: skillArr,
        rating: profile.rating ?? 5, tours: profile.tours ?? 0,
        match: profile.match ?? 80, status: 'active',
...
async function syncGuestProfile(user: CurrentUser): Promise<void> {
  ...
  await AsyncStorage.setItem('@app_profile', JSON.stringify({
    ...old,
    name:        user.name,
    email:       user.email,
    phone:       user.phone,
...
async function syncGuideStatusToPublic(
  guideId: string, status: GuideStatus, hideFromList: boolean
): Promise<void> {
  try {
    const raw = await AsyncStorage.getItem('@app_guides');
    if (!raw) return;
...
async function addGuideToPublic(guideId: string, req: GuideRequest): Promise<void> {
  try {
    const raw = await AsyncStorage.getItem('@app_guides');
    const list = raw ? JSON.parse(raw) : [];
    if (list.find((g: any) => g.id === guideId)) return; // already exists
    list.unshift({
      id: guideId, name: req.name,
      location: 'Việt Nam', experience: req.experience || '1 năm',
      skills: req.skills.split(',').map((s: string) => s.trim()).filter(Boolean),
      rating: 5, tours: 0, match: 80, status: 'active',
