...
export default function GuestVouchers() {
  ...
  const addCode = async () => {
    ...
    if (found.usedCount >= found.limit) {
      Alert.alert("Hết lượt", "Mã voucher này đã được sử dụng hết lượt."); return;
    }
    const newV: Voucher = {
      id: `v${Date.now()}`,
      code,
      ...
      maxDiscount: found.maxDiscount ?? 0,
      desc: found.description ?? "Voucher từ hệ thống",
      expiry: found.expiry ?? "31/12/2026",
      used: false,
      source: found.source === "loyalty" ? "loyalty" : "promo",
      color: found.color ?? "#4f7cff",
    };
          ...
          return (
            <View key={v.id} style={[s.card, inactive && s.cardInactive]}>
              ...
              <View style={{ flex: 1, padding: 12 }}>
                ...
                {/* Chỉ 1 button duy nhất */}
                {!inactive && (
                  <TouchableOpacity
                    style={[s.useBtn, { backgroundColor: v.color }]}
                    onPress={async () => {
                      await AsyncStorage.setItem("@active_voucher", JSON.stringify({
                        code: v.code, type: v.type, value: v.value,
                        minOrder: v.minOrder ?? 0, maxDiscount: v.maxDiscount ?? 0,
                        desc: v.desc, voucherId: v.id,
                      })).catch(() => {});
                      router.push("/guest_booking_flow" as any);
                    }}
                  >
...
const s = StyleSheet.create({
  ...
  cardBottom:    { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 6, marginBottom: 4 },
  ...
  useBtn:        { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, borderRadius: 10, paddingVertical: 10, marginTop: 4 },
