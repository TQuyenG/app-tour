...
export default function AdminGuideRequestsScreen() {
    ...
    Alert.alert("Phê duyệt", `Xác nhận phê duyệt ${req.name} làm HDV?`, [
      { text: "Hủy", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          Alert.alert(
            "Đã phê duyệt",
            `${req.name} đã có thêm quyền Hướng dẫn viên.`,
          );
        },
      },
    ]);
  ...
  const onReject = async () => {
    if (!detail) return;
    await rejectGuideRequest(
      detail.id,
      rejectNote.trim() || "Không đáp ứng yêu cầu",
    );
    getPendingGuideRequests().then(setRequests);
  ...
  const statusMeta = (s: string) =>
    ...
    )[s] ?? { label: s, color: "#7a8cc2", bg: "#f3f7ff" };
      ...
      {visible.length === 0 && (
        <View style={st.emptyCard}>
          ...
          <Text style={st.emptyTxt}>
            Không có yêu cầu {filter === "pending" ? "đang chờ" : ""}.
          </Text>
        </View>
      )}
            ...
            <View style={st.cardBody}>
              <View style={st.cardTop}>
                ...
                <View style={[st.statusBadge, { backgroundColor: meta.bg }]}>
                  <Text style={[st.statusTxt, { color: meta.color }]}>
                    {meta.label}
                  </Text>
                </View>
              </View>
                ...
                <View style={st.actionRow}>
                  ...
                  >
                    <Ionicons
                      ...
                      color="#16a34a"
                    />
                    <Text style={[st.actionBtnTxt, { color: "#16a34a" }]}>
                      Phê duyệt
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={[st.actionBtnTxt, { color: "#ef4444" }]}>
                      Từ chối
                    </Text>
                  </TouchableOpacity>
                </View>
            ...
            {detail && (
              ...
              >
                ...
                <View style={st.detailRow}>
                  <Text style={st.detailLbl}>Email</Text>
                  <Text style={st.detailVal}>{detail.email}</Text>
                </View>
                <View style={st.detailRow}>
                  <Text style={st.detailLbl}>SĐT</Text>
                  <Text style={st.detailVal}>{detail.phone}</Text>
                </View>
                <View style={st.detailRow}>
                  <Text style={st.detailLbl}>Kinh nghiệm</Text>
                  <Text style={st.detailVal}>{detail.experience}</Text>
                </View>
                <View style={st.detailRow}>
                  <Text style={st.detailLbl}>Kỹ năng</Text>
                  <Text style={st.detailVal}>{detail.skills}</Text>
                </View>
                <View style={st.detailRow}>
                  <Text style={st.detailLbl}>Ngôn ngữ</Text>
                  <Text style={st.detailVal}>{detail.languages}</Text>
                </View>
                <Text style={st.detailLbl}>Giới thiệu</Text>
                <View style={st.bioBox}>
                  <Text style={st.bioTxt}>{detail.bio}</Text>
                </View>
                {detail.status === "pending" && (
                  <View style={st.modalActions}>
                    <TouchableOpacity
                      style={st.modalApproveBtn}
...
const st = StyleSheet.create({
  ...
  sub: { color: "#7a8cc2", marginBottom: 16 },
  ...
  cardMeta: { color: "#7a8cc2", fontSize: 12, marginBottom: 3 },
  cardSkills: { color: "#5f73a9", fontSize: 12, marginBottom: 3 },
  cardDate: { color: "#c0cbe8", fontSize: 11 },
  ...
  detailLbl: { color: "#7a8cc2", fontSize: 13 },
  detailVal: {
    color: "#1f2a58",
    fontWeight: "600",
    fontSize: 13,
  ...
  bioTxt: { color: "#5f73a9", lineHeight: 20 },
