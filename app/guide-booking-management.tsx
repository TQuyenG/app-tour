...
interface Booking {
  ...
  tourName: string; date: string; duration: string;
  guests: number; price: string; status: BookingStatus;
  note: string; createdAt: string;
}

const SEED: Booking[] = [
  { id: '1', customerName: 'Trần Thị B', customerPhone: '0901 111 222', tourName: 'Tour Núi Bà Đen', date: '20/03/2025', duration: '1 ngày', guests: 4, price: '1200000', status: 'pending', note: 'Khách cần hướng dẫn tiếng Anh.', createdAt: '5 giờ trước' },
  { id: '2', customerName: 'Lê Văn C', customerPhone: '0912 333 444', tourName: 'Tour Đà Lạt 2N1Đ', date: '22/03/2025', duration: '2 ngày 1 đêm', guests: 2, price: '2800000', status: 'accepted', note: '', createdAt: 'Hôm qua' },
  { id: '3', customerName: 'Nguyễn Thị D', customerPhone: '0933 555 666', tourName: 'Tour Mũi Né biển', date: '15/03/2025', duration: '2 ngày 1 đêm', guests: 6, price: '3600000', status: 'done', note: 'Khách rất hài lòng.', createdAt: '3 ngày trước' },
  { id: '4', customerName: 'Phạm Văn E', customerPhone: '0944 777 888', tourName: 'Tour Vũng Tàu', date: '10/03/2025', duration: '1 ngày', guests: 3, price: '900000', status: 'cancelled', note: 'Khách hủy do thời tiết.', createdAt: '5 ngày trước' },
  { id: '5', customerName: 'Hoàng Thị F', customerPhone: '0955 999 000', tourName: 'Trekking Langbiang', date: '25/03/2025', duration: '1 ngày', guests: 5, price: '1500000', status: 'pending', note: '', createdAt: '1 giờ trước' },
];
...
export default function GuideBookingManagement() {
  ...
  const persist = useCallback(async (data: Booking[]) => {
    setBookings(data); await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(data)).catch(() => {});
  }, []);

  const updateStatus = (id: string, status: BookingStatus) => {
    const b = bookings.find(x => x.id === id); if (!b) return;
    Alert.alert('Xác nhận', `Chuyển booking "${b.customerName}" sang "${STATUS_MAP[status].label}"?`, [
      { text: 'Hủy', style: 'cancel' },
      { text: 'Đồng ý', onPress: () => persist(bookings.map(x => x.id === id ? { ...x, status } : x)) },
    ]);
  };

  const filtered = filter === 'all' ? bookings : bookings.filter(b => b.status === filter);
  const pending = bookings.filter(b => b.status === 'pending').length;
  const totalEarned = bookings.filter(b => b.status === 'done').reduce((s, b) => s + Number(b.price), 0);
  ...
  return (
    <View style={s.container}>
      <View style={s.topBar}>
        ...
        {pending > 0 && <View style={s.pendingBadge}><Text style={s.pendingTxt}>{pending} mới</Text></View>}
      </View>

      <View style={s.statsRow}>
        {[{ l: 'Tổng', v: bookings.length, c: '#1a2f7a' }, { l: 'Chờ xác nhận', v: pending, c: '#d97706' }, { l: 'Đang dẫn', v: bookings.filter(b => b.status === 'ongoing').length, c: '#a855f7' }, { l: 'Thu nhập', v: `${(totalEarned / 1000000).toFixed(1)}tr`, c: '#16a34a' }].map((item, i, arr) => (
      ...
      <ScrollView contentContainerStyle={s.list}>
        {filtered.length === 0 && (
          <View style={s.emptyWrap}><Ionicons name="calendar-outline" size={56} color="#c0cbe8" /><Text style={s.emptyText}>Không có booking nào</Text></View>
        )}
              ...
              <View style={s.cardBody}>
                <View style={s.cardTopRow}>
                  <View style={s.custRow}>
                    <View style={s.custAvatar}><Ionicons name="person" size={14} color="#fff" /></View>
                    <View>
                ...
                <View style={s.metaRow}>
                  <Ionicons name="calendar-outline" size={12} color="#8ea0d6" /><Text style={s.metaTxt}>{booking.date}</Text>
                  <Text style={s.dot}>·</Text>
                  <Ionicons name="people-outline" size={12} color="#8ea0d6" /><Text style={s.metaTxt}>{booking.guests} khách</Text>
                  <Text style={s.dot}>·</Text>
                  <Text style={s.metaTxt}>{booking.duration}</Text>
                </View>
                <View style={s.cardBottom}>
                  <Text style={s.price}>{Number(booking.price).toLocaleString('vi-VN')}đ</Text>
                  <Text style={s.createdAt}>{booking.createdAt}</Text>
                </View>
                {isOpen && (
                  <View style={s.expandSection}>
                    {!!booking.note && <View style={s.noteBox}><Ionicons name="document-text-outline" size={13} color="#4f7cff" /><Text style={s.noteTxt}>{booking.note}</Text></View>}
                    <View style={s.actionRow}>
                      {booking.status === 'pending' && <>
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#dcfce7' }]} onPress={() => updateStatus(booking.id, 'accepted')}>
                          <Ionicons name="checkmark-circle-outline" size={14} color="#16a34a" /><Text style={[s.actionTxt, { color: '#16a34a' }]}>Nhận tour</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#fee2e2' }]} onPress={() => updateStatus(booking.id, 'cancelled')}>
                          <Ionicons name="close-circle-outline" size={14} color="#dc2626" /><Text style={[s.actionTxt, { color: '#dc2626' }]}>Từ chối</Text>
                        </TouchableOpacity>
                      </>}
                      {booking.status === 'accepted' && (
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#f3e8ff' }]} onPress={() => updateStatus(booking.id, 'ongoing')}>
                          <Ionicons name="play-circle-outline" size={14} color="#a855f7" /><Text style={[s.actionTxt, { color: '#a855f7' }]}>Bắt đầu dẫn</Text>
                        </TouchableOpacity>
                      )}
                      {booking.status === 'ongoing' && (
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#dcfce7' }]} onPress={() => updateStatus(booking.id, 'done')}>
                          <Ionicons name="flag-outline" size={14} color="#16a34a" /><Text style={[s.actionTxt, { color: '#16a34a' }]}>Hoàn thành</Text>
                        </TouchableOpacity>
                      )}
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#eaf0ff' }]} onPress={() => { setNoteText(booking.note); setNoteModal(booking); }}>
                        <Ionicons name="create-outline" size={14} color="#4f7cff" /><Text style={[s.actionTxt, { color: '#4f7cff' }]}>Ghi chú</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#e8eeff' }]} onPress={() => Alert.alert('Gọi điện', `${booking.customerName}\n${booking.customerPhone}`)}>
                        <Ionicons name="call-outline" size={14} color="#2856d6" /><Text style={[s.actionTxt, { color: '#2856d6' }]}>Gọi</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                )}
        ...
        <KeyboardAvoidingView style={s.overlay} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          <View style={s.sheet}>
            ...
            <View style={s.modalBody}>
              ...
              <TouchableOpacity style={s.saveBtn} onPress={() => { if (noteModal) { persist(bookings.map(x => x.id === noteModal.id ? { ...x, note: noteText } : x)); setNoteModal(null); } }}>
                <Ionicons name="checkmark-circle-outline" size={18} color="#fff" /><Text style={s.saveBtnTxt}>Lưu ghi chú</Text>
              </TouchableOpacity>
            </View>
          </View>
...
const s = StyleSheet.create({
  ...
  cardTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6, marginBottom: 6 },
  custRow: { flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 },
  custAvatar: { width: 32, height: 32, borderRadius: 10, backgroundColor: '#2856d6', alignItems: 'center', justifyContent: 'center' },
  custName: { color: '#1a2f7a', fontWeight: '700', fontSize: 14 },
  custPhone: { color: '#7a8cc2', fontSize: 11, marginTop: 1 },
  badge: { borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  badgeTxt: { fontSize: 11, fontWeight: '700' },
  tourName: { color: '#2856d6', fontWeight: '700', fontSize: 13, marginBottom: 6 },
  ...
  noteBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 6, backgroundColor: '#f0f4ff', borderRadius: 10, padding: 10, marginBottom: 10 },
