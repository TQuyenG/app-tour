 ...
 * FIX:
 * 1. Lịch sử điểm đọc từ @guest_loyalty_history (đúng key active_tour_tracking ghi vào)
 * 2. Điểm đọc từ @app_profile.loyaltyPoints (đúng key active_tour_tracking ghi vào)
 * 3. Hiển thị rõ loại giao dịch: earn (cộng xanh) / redeem (trừ đỏ)
 */
...
export default function GuestLoyaltyScreen() {
  ...
  useFocusEffect(useCallback(() => {
    const loadData = async () => {
      try {
        // ✅ Đọc điểm từ @app_profile.loyaltyPoints (đúng key active_tour_tracking ghi vào)
        const pRaw = await AsyncStorage.getItem("@app_profile");
        if (pRaw) {
          const p = JSON.parse(pRaw);
          const currentPoints = Number(p.loyaltyPoints) || 0;
        ...
        // Đọc lịch sử từ @guest_loyalty_history (đúng key active_tour_tracking ghi vào)
        const hRaw = await AsyncStorage.getItem("@guest_loyalty_history");
        setHistory(hRaw ? JSON.parse(hRaw) : []);

        // Đọc voucher đổi điểm từ admin
        ...
        console.error('GuestLoyalty loadData error:', e);
      }
    };
    loadData();
  }, []));
  ...
  const handleRedeemClick = async (voucher: any) => {
    ...
    if (points < cost) {
      Alert.alert("Không đủ điểm", `Bạn cần thêm ${(cost - points).toLocaleString('vi-VN')} điểm nữa.`);
      return;
    }
  ...
  const executeRedeem = async () => {
    ...
    try {
      ...
      const newHistoryEntry = {
        ...
        type: "redeem",
        points: cost,
        desc: `Đổi voucher: ${voucher.title || voucher.code}`,
      ...
      console.error('executeRedeem error:', e);
    }
  };
  ...
  return (
    <View style={s.screen}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.card, s.tierCard]}>
          ...
          {tier.name !== "Kim Cương" && (
            <View style={s.progressWrap}>
              ...
              <Text style={s.progressTxt}>Cần {Math.max(tier.maxPoints - points + 1, 0).toLocaleString('vi-VN')} điểm nữa để thăng hạng</Text>
            </View>
          )}
        </View>
            ...
            {rewards.map((v, i) => (
              <View key={i} style={s.redeemCard}>
                <View style={s.redeemTop}>
                  <Text style={s.redeemValue}>{v.type === 'percent' ? `${v.discountValue}%` : `${Math.floor(v.discountValue / 1000)}K`}</Text>
        ...
        {/* Lịch sử điểm - hiển thị đầy đủ earn/redeem */}
        <Text style={s.sectionTitle}>Lịch sử điểm</Text>
        {history.length === 0 ? (
          <View style={s.emptyHistory}>
            <Ionicons name="receipt-outline" size={48} color="#c0cbe8" />
            <Text style={s.emptyHistoryTxt}>Chưa có giao dịch điểm nào.</Text>
            <Text style={s.emptyHistoryHint}>Hoàn thành tour để nhận điểm thưởng!</Text>
          </View>
        ) : (
          history.map(item => (
            <View key={item.id} style={s.historyCard}>
              <View style={[s.historyIcon, { backgroundColor: item.type === "earn" ? "#eaf0ff" : "#fee2e2" }]}>
                <Ionicons
                  ...
                  size={22}
                  color={item.type === "earn" ? "#4f7cff" : "#ef4444"}
                />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={s.historyDesc} numberOfLines={2}>{item.desc}</Text>
                <Text style={s.historyDate}>{formatDate(item.date)}</Text>
              </View>
              <Text style={[s.historyPoints, { color: item.type === "earn" ? "#4f7cff" : "#ef4444" }]}>
                {item.type === "earn" ? "+" : "-"}{(item.points || 0).toLocaleString('vi-VN')}
              </Text>
            </View>
          ))
        )}
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalBox}>
            ...
            <Text style={s.modalSub}>
              Dùng <Text style={{ color: '#4f7cff', fontWeight: '900' }}>{confirmModal?.cost?.toLocaleString('vi-VN')} điểm</Text> để lấy voucher:{"\n"}
              <Text style={{ color: '#1f2a58', fontWeight: '700' }}>{confirmModal?.voucher?.title}</Text>?
            </Text>
            <Text style={{ fontSize: 13, color: '#94a3b8', marginBottom: 20 }}>Còn lại: {(points - (confirmModal?.cost || 0)).toLocaleString('vi-VN')} điểm</Text>
            <View style={s.modalBtnRow}>
              <TouchableOpacity style={s.modalCancelBtn} onPress={() => setConfirmModal(null)}>
                <Text style={s.modalCancelTxt}>Hủy</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.modalSubmitBtn} onPress={executeRedeem}>
                <Text style={s.modalSubmitTxt}>Đồng ý Đổi</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  emptyHistoryTxt: { color: '#94a3b8', fontSize: 15, fontWeight: '600', marginTop: 12 },
  emptyHistoryHint: { color: '#c0cbe8', fontSize: 13, marginTop: 6 },
  ...
  historyDesc: { fontSize: 14, fontWeight: "700", color: "#1f2a58", lineHeight: 20 },
  historyDate: { fontSize: 12, color: "#94a3b8", marginTop: 4 },
  historyPoints: { fontSize: 17, fontWeight: "900" },
  ...
  modalSub: { fontSize: 14, color: "#64748b", textAlign: "center", marginBottom: 8, lineHeight: 22 },
