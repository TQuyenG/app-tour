...
export default function GuestChatDetail() {
  ...
  const loadChat = async () => {
    ...
    if (type === 'guide') {
      ...
      if (staffRaw) {
        const list = JSON.parse(staffRaw);
        const current = list.find((c: any) => c.id === bookingId);
        if (current) {
          setMessages(current.messages || []);
          setTitle(current.staffName || "Tổng đài CSKH");
        }
      }
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    setInputText("");
    const newMsg = { from: 'guest', text: txt, time: new Date().toLocaleTimeString('vi-VN', {hour:'2-digit', minute:'2-digit'}) };
    ...
    if (type === 'guide') {
      ...
      if (staffRaw) {
        const list = JSON.parse(staffRaw);
        const updated = list.map((c:any) => c.id === bookingId ? { ...c, messages: [...(c.messages||[]), newMsg], lastMessage: txt } : c);
        await AsyncStorage.setItem("@guest_staff_chats", JSON.stringify(updated));
      }
    }
    await loadChat();
  };
  ...
  const handleBack = () => {
    if (router.canGoBack()) {
      ...
      router.replace("/guest_chat_list"); // Trả về Danh sách Chat Khách
    }
  };
    ...
    <KeyboardAvoidingView style={s.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <FlatList ref={flatListRef} data={messages} keyExtractor={(_, i) => `msg_${i}`} contentContainerStyle={s.list} onLayout={() => flatListRef.current?.scrollToEnd({ animated: false })}
        renderItem={({ item }) => {
          ...
          return (
            <View style={[s.msgRow, isGuest ? s.msgRight : s.msgLeft]}>
              <View style={[s.bubble, isGuest ? s.bubbleGuest : s.bubbleOther]}>
                <Text style={[s.msgTxt, isGuest ? { color: '#fff' } : { color: '#1f2a58' }]}>{item.text}</Text>
              </View>
      ...
      <View style={[s.inputArea, { paddingBottom: Math.max(insets.bottom, Math.round(12*scale)) }]}>
        ...
        <View style={s.textInputWrapper}>
          <TextInput style={s.textInput} value={inputText} onChangeText={setInputText} placeholder="Tin nhắn..." placeholderTextColor="#94a3b8" multiline />
        </View>
