 ...
 * SẠCH 100% DỮ LIỆU MẪU - LUỒNG ĐỒNG BỘ
 */
...
const FILTERS = ["Tất cả", "Chờ duyệt", "Đang xử lý", "Đã hoàn", "Từ chối"];
...
export default function StaffRefundManagement() {
  ...
  const handleUpdateStatus = (req: RefundRequest, newStatus: RefundRequest["status"]) => {
    if (newStatus === "rejected" && !rejectReason.trim()) {
      Alert.alert("Lỗi", "Vui lòng nhập lý do từ chối hoàn tiền.");
      return;
    }

    const label = STATUS_META[newStatus].label;
    Alert.alert("Xác nhận", `Cập nhật yêu cầu #${req.id} sang "${label}"?`, [
      { text: "Hủy", style: "cancel" },
      {
        text: "Xác nhận",
        style: newStatus === "rejected" ? "destructive" : "default",
        onPress: async () => {
          const updated = refunds.map(r => r.id === req.id ? { ...r, status: newStatus } : r);
          await persist(updated);
          if (detail?.id === req.id) setDetail({ ...detail, status: newStatus });
          setShowDetail(false);
          setRejectReason("");
          Alert.alert("Thành công", `Đã cập nhật trạng thái thành ${label}`);
        }
      }
    ]);
  };
  ...
  const filtered = refunds.filter(r => {
    ...
    if (filter === "Đang xử lý") return r.status === "processing";
                ...
                {detail.status === "pending" && (
                  <View style={s.actionSection}>
                    ...
                    <View style={s.actionBtns}>
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#16a34a" }]} onPress={() => handleUpdateStatus(detail, "approved")}>
                        <Text style={s.actionBtnTxt}>Duyệt hoàn tiền</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#0284c7" }]} onPress={() => handleUpdateStatus(detail, "processing")}>
                        <Text style={s.actionBtnTxt}>Chuyển kế toán</Text>
                      </TouchableOpacity>
                    </View>
                    <View style={s.rejectBox}>
                      ...
                      />
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#fff", borderWidth: 1, borderColor: "#dc2626", marginTop: 8 }]} onPress={() => handleUpdateStatus(detail, "rejected")}>
