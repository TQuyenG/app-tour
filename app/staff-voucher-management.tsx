...
export default function StaffVoucherManagement() {
  ...
  const handleSendDirect = async () => {
    ...
    const newVoucher = { id: Date.now().toString(), code: `CSKH${Math.random().toString(36).substring(2, 8).toUpperCase()}`, value: selectedTemplate.value, type: selectedTemplate.type, targetUserId: selectedUser.id, targetUserName: selectedUser.name, targetUserPhone: selectedUser.phone, reason: customReason || selectedTemplate.reason, createdAt: new Date().toLocaleDateString("vi-VN"), used: false };
    ...
    showAlert("Thành công", `Đã gửi Voucher thành công cho khách hàng ${selectedUser.name}`, "success");
  };
  ...
  return (
    <View style={s.container}>
        ...
        <View style={s.userModalOverlay}>
          <View style={s.sheet}>
            ...
            <View style={{ padding: 16 }}>
              ...
              <FlatList data={filteredUsers} keyExtractor={item => item.id} style={{ maxHeight: 400, marginTop: 10 }}
                renderItem={({ item }) => (
                  <TouchableOpacity style={s.userItem} onPress={() => { setSelectedUser(item); setShowUserModal(false); }}>
                    ...
                    <View><Text style={s.userName}>{item.name}</Text><Text style={s.userPhone}>{item.phone || item.email}</Text></View>
                  </TouchableOpacity>
                )}
              />
            </View>
      ...
      <View style={s.tabContainer}>
        <TouchableOpacity style={[s.tabBtn, activeTab === "system" && s.tabActive]} onPress={() => setActiveTab("system")}><Text style={[s.tabTxt, activeTab === "system" && s.tabTxtActive]}>Mã Hệ thống</Text></TouchableOpacity>
        <TouchableOpacity style={[s.tabBtn, activeTab === "direct" && s.tabActive]} onPress={() => setActiveTab("direct")}><Text style={[s.tabTxt, activeTab === "direct" && s.tabTxtActive]}>Tặng cá nhân</Text></TouchableOpacity>
      </View>
      ...
      {activeTab === "system" ? (
        ...
        <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
          <View style={s.formCard}>
            <Text style={s.sectionTitle}>1. Chọn Khách hàng nhận</Text>
            <TouchableOpacity style={s.selectUserBtn} onPress={() => setShowUserModal(true)}>
              {selectedUser ? (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}><Ionicons name="checkmark-circle" size={20} color="#16a34a" /><Text style={{ color: "#1f2a58", fontWeight: "700" }}>{selectedUser.name} - {selectedUser.phone}</Text></View>
              ) : (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}><Ionicons name="person-add-outline" size={20} color="#7a8cc2" /><Text style={{ color: "#7a8cc2" }}>Bấm để chọn khách hàng...</Text></View>
              )}
            </TouchableOpacity>
            
            <Text style={[s.sectionTitle, { marginTop: 16 }]}>2. Chọn Mức Bồi Thường</Text>
            <View style={s.templateGrid}>
              {VOUCHER_TEMPLATES.map((t, i) => (
                <TouchableOpacity key={i} style={[s.templateCard, selectedTemplate?.label === t.label && { borderColor: t.color, backgroundColor: t.color + "10" }]} onPress={() => setSelectedTemplate(t)}>
                  <Ionicons name={t.icon as any} size={24} color={t.color} /><Text style={s.tLabel}>{t.label}</Text><Text style={s.tSub}>{t.sublabel}</Text>
                </TouchableOpacity>
              ))}
            </View>
          ...
          {directVouchers.map(v => (
            <View key={v.id} style={s.historyCard}>
              ...
              <View style={s.historyBottom}><Text style={s.historyVal}>{v.type === "fixed" ? v.value.toLocaleString("vi-VN")+"đ" : v.value+"%"}</Text><Text style={s.historyDate}>{v.createdAt}</Text></View>
            </View>
          ))}
        </ScrollView>
      )}
...
const s = StyleSheet.create({
  ...
  historyGuest: { color: "#5f73a9", fontSize: 13, marginBottom: 8 },
