...
const SEED_EVENTS: ScheduleEvent[] = [
  {
    ...
    time: "07:00",
    type: "tour",
    note: "4 khách, xe đón tại Bến xe Miền Đông.",
  },
  {
    id: "2",
    date: "22/03/2025",
    ...
    time: "06:00",
    type: "tour",
    note: "2 khách, nghỉ tại khách sạn Đà Lạt Palace.",
  },
  {
    id: "3",
    date: "25/03/2025",
...
const EMPTY_EVENT: Omit<ScheduleEvent, "id"> = {
  date: "",
  title: "",
  time: "",
  type: "block",
...
export default function GuideScheduleManagement() {
    ...
    AsyncStorage.getItem(STORAGE_KEY)
      .then((raw) => {
        if (raw) {
          ...
          setDays(d.days || genDays());
          setEvents(d.events || SEED_EVENTS);
        } else {
          setEvents(SEED_EVENTS);
          persist(genDays(), SEED_EVENTS);
        }
      })
  ...
  const persist = useCallback(async (d: ScheduleDay[], e: ScheduleEvent[]) => {
    ...
    await AsyncStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ days: d, events: e }),
    ).catch(() => {});
  }, []);
  ...
  const handleSave = () => {
    if (!form.title.trim() || !form.date.trim()) {
      Alert.alert("Thiếu thông tin", "Vui lòng điền Tiêu đề và Ngày.");
      return;
    }
    const updated = editingEvent
      ? events.map((e) =>
          e.id === editingEvent.id ? { ...form, id: editingEvent.id } : e,
        )
      : [...events, { ...form, id: Date.now().toString() }];
    persist(days, updated);
    setModalVisible(false);
  };
  const handleDeleteEvent = (ev: ScheduleEvent) =>
    Alert.alert("Xóa sự kiện", `Xóa "${ev.title}"?`, [
  ...
  const f = (k: keyof Omit<ScheduleEvent, "id">, v: string) =>
    setForm((p) => ({ ...p, [k]: v }));
  ...
  const calendarCells: (ScheduleDay | null)[] = [
      ...
      <View style={s.container}>
        <View style={[s.topBar, { paddingTop: insets.top + 10 }]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => openAddEvent(selectedDate || undefined)}
          >
        ...
        <ScrollView contentContainerStyle={s.content}>
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[
                          s.calDayNum,
                          { color: isSelected ? st.color : "#1a2f7a" },
                        ]}
                      >
                        {dayNum}
                      </Text>
          ...
          <Text style={s.hintTxt}>
            💡 Nhấn giữ ngày để thay đổi trạng thái rảnh / bận / có tour
          </Text>
          ...
          <View style={s.statsStrip}>
            {[
              {
                label: "Ngày rảnh",
                val: days.filter((d) => d.status === "available").length,
                color: "#16a34a",
              },
              {
                label: "Ngày bận",
                val: days.filter((d) => d.status === "busy").length,
                color: "#d97706",
              },
              {
                label: "Có tour",
                val: days.filter((d) => d.status === "booked").length,
                color: "#2856d6",
              },
              ...
              <View
                key={item.label}
                style={[s.stripItem, i < arr.length - 1 && s.stripBorder]}
              >
                <Text style={[s.stripNum, { color: item.color }]}>
                  {item.val}
                </Text>
          ...
          {selectedDate && (
            <View style={s.section}>
              <View style={s.sectionHeader}>
                ...
                <TouchableOpacity
                  style={s.addEventBtn}
                  onPress={() => openAddEvent(selectedDate)}
                >
                  <Ionicons name="add" size={14} color="#2856d6" />
                  <Text style={s.addEventTxt}>Thêm</Text>
                </TouchableOpacity>
              </View>
              {selectedEvents.length === 0 && (
                <Text style={s.noEventTxt}>
                  Không có sự kiện nào. Nhấn + để thêm.
                </Text>
              )}
              {selectedEvents.map((ev) => {
                ...
                return (
                  <View
                    key={ev.id}
                    style={[s.eventCard, { borderLeftColor: tp.color }]}
                  >
                    <View style={[s.eventTypeDot, { backgroundColor: tp.bg }]}>
                      <Ionicons name={tp.icon} size={14} color={tp.color} />
                    </View>
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={s.eventMeta}>
                        <Ionicons
                          ...
                          color="#8ea0d6"
                        />
                        <Text style={s.eventMetaTxt}>{ev.time}</Text>
                      </View>
            ...
            {events.slice(0, 5).map((ev) => {
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={s.eventMeta}>
                      <Ionicons
                        ...
                        color="#8ea0d6"
                      />
                      <Text style={s.eventMetaTxt}>{ev.date}</Text>
                  ...
                  <View style={[s.badge, { backgroundColor: tp.bg }]}>
                    <Text style={[s.badgeTxt, { color: tp.color }]}>
                      {tp.label}
                    </Text>
                  </View>
                </View>
            ...
            <View style={s.sheet}>
              ...
              >
                ...
                <TextInput
                  ...
                  placeholder="VD: Tour Đà Lạt"
                  placeholderTextColor="#b0bdd8"
                />
                <FL t="Ngày (dd/mm/yyyy) *" />
                <TextInput
                  style={s.input}
                  value={form.date}
                ...
                <View style={s.optionRow}>
                  {(
                    Object.keys(EVENT_TYPE_MAP) as Array<
                      keyof typeof EVENT_TYPE_MAP
                    >
                  ).map((k) => {
                    ...
                    return (
                      ...
                      >
                        <Ionicons
                          name={tp.icon}
                          size={13}
                          color={form.type === k ? tp.color : "#7a8cc2"}
                        />
                        <Text
                          style={[
                            s.typeChipTxt,
                            form.type === k && { color: tp.color },
                          ]}
                        >
                          {tp.label}
                        </Text>
                ...
                <FL t="Ghi chú" />
                <TextInput
                  style={[s.input, { minHeight: 80, paddingTop: 12 }]}
                  value={form.note}
        ...
        >
          {(
            ...
            const isActive = tab.route === "/guide-schedule-management";
            return (
              <TouchableOpacity
                key={tab.route}
                style={tabSt.item}
