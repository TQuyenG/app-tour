...
const MENU_ITEMS = [
  ...
  { icon: 'notifications-outline', label: 'Thông báo hệ thống', route: '/notifications', color: '#4f7cff' },
  { icon: 'shield-checkmark-outline', label: 'Bảo mật & Quyền hạn', route: '/settings', color: '#ef4444' },
  { icon: 'create-outline', label: 'Cập nhật thông tin', route: '/admin-profile-edit', color: '#4f7cff' },
] as const;
...
export default function AdminProfile() {
  ...
  return (
    <ScrollView
      style={styles.screen}
      contentContainerStyle={[styles.content, { paddingTop: insets.top + 14, paddingBottom: 100 }]}>
      ...
      <View style={styles.statsCard}>
        ...
        <View style={styles.statsGrid}>
          {[
            { label: 'Người dùng', value: profile.stats.users, icon: 'people-outline', color: '#4f7cff' },
            { label: 'Booking', value: profile.stats.bookings, icon: 'receipt-outline', color: '#22c55e' },
            { label: 'Doanh thu', value: profile.stats.revenue, icon: 'cash-outline', color: '#f59e0b' },
            { label: 'Hài lòng', value: profile.stats.satisfaction, icon: 'happy-outline', color: '#a855f7' },
          ].map((s, i) => (
            <View key={i} style={[styles.statItem, { backgroundColor: s.color + '15' }]}>
      ...
      {/* Logout */}
      <TouchableOpacity style={styles.logoutBtn} onPress={() => router.push('/login' as any)}>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    borderWidth: 2, borderColor: '#dfe7ff',
  },
  profileInfo: { flex: 1 },
  ...
  roleBadge: {
    ...
    marginTop: 5, backgroundColor: '#edf2ff', alignSelf: 'flex-start',
  ...
  infoCard: {
    backgroundColor: '#fff', borderRadius: 16,
    borderWidth: 1, borderColor: '#e4ebff', marginBottom: 18,
  },
  ...
  logoutBtn: {
    ...
    padding: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10,
  },
