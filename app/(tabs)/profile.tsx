 ...
 * FIX: Thêm popup xác nhận đăng xuất + icon logout trên header
 */
...
export default function ProfileScreen() {
  ...
  const handleCheckIn = async () => {
    try {
      ...
      history.unshift({ id: `h-${Date.now()}`, type: "earn", points: 20, desc: "Điểm danh hàng ngày", date: new Date().toISOString() });
  ...
  // ← MỚI: thực sự đăng xuất sau khi xác nhận
  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={[s.content, { paddingTop: insets.top + Math.round(14 * scale), paddingBottom: Math.round(100 * scale) + insets.bottom }]}>
      ...
      <View style={s.profileCard}>
        {profile.avatarUrl ? (
          ...
          <View style={[s.avatar, { backgroundColor: profile.avatarColor }]}>
            <Text style={s.avatarInitial}>{(profile.name || 'U').trim().charAt(0).toUpperCase()}</Text>
          </View>
        )}
        ...
        {/* ← icon logout trên header card */}
        <TouchableOpacity onPress={() => setLogoutModal(true)} style={s.headerLogoutBtn}>
          <Ionicons name="log-out-outline" size={Math.round(22 * scale)} color="#ef4444" />
        </TouchableOpacity>
      </View>
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={() => router.push('/guest_loyalty')}>
          ...
          <View style={s.crmRow}>
            ...
            <View style={[s.tierBadge, { backgroundColor: tierColor + '22' }]}>
              <Text style={[s.tierTxt, { color: tierColor }]}>{profile.loyaltyTier}</Text>
            </View>
          </View>
          <View style={s.progressWrap}>
            <View style={s.progressBg}>
              <View style={[s.progressFill, { width: `${Math.min((profile.loyaltyPoints % 1000) / 10, 100)}%` as any }]} />
            </View>
      ...
      {([
        ...
        <TouchableOpacity key={idx} style={s.menuItem} onPress={item.action ? item.action : () => router.push(item.href as any)}>
          <View style={[s.menuIcon, { backgroundColor: (item as any).color + '18' }]}>
            <Ionicons name={item.icon as any} size={Math.round(20 * scale)} color={(item as any).color} />
          </View>
      ...
      <Modal visible={logoutModal} transparent animationType="fade">
        <View style={s.popupOverlay}>
          <View style={s.logoutPopup}>
            ...
            <Text style={s.logoutPopupSub}>Bạn có chắc chắn muốn đăng xuất khỏi tài khoản không?</Text>
          ...
          <View style={s.sheet}>
            ...
            <ScrollView contentContainerStyle={s.mbody}>
              ...
              <TouchableOpacity style={s.saveBtn} onPress={handleSave}>
                <Text style={s.saveBtnTxt}>{saving ? 'Đang lưu...' : 'Lưu thay đổi'}</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
...
const getStyles = (scale: number) => {
  ...
  return StyleSheet.create({
    ...
    headerLogoutBtn: { padding: sz(6), borderRadius: sz(10), backgroundColor: '#fef2f2' },
