 ...
 * Admin quản lý và phân xử Khiếu nại (Đồng bộ kho @app_complaints cùng Staff)
 * Bổ sung trạng thái "Cần Admin can thiệp" (Escalated)
 */
...
const STATUS_META: Record<string, { label: string; color: string; bg: string }> = {
  pending:       { label: "Mới tạo",             color: "#64748b", bg: "#f1f5f9" },
  investigating: { label: "Staff đang xử lý",    color: "#d97706", bg: "#fef9c3" },
  escalated:     { label: "CẦN ADMIN XỬ LÝ",     color: "#e11d48", bg: "#ffe4e6" }, // Trạng thái đặc biệt cho Admin
...
const FILTERS = ["Tất cả", "Cần Admin can thiệp", "Staff đang xử lý", "Đã giải quyết"];
...
export default function AdminComplaintsScreen() {
  ...
  const [filter, setFilter] = useState("Cần Admin can thiệp"); // Mặc định Admin vào là xem các ca khó trước
  ...
  useEffect(() => {
    if (initialSearch) {
      setSearch(initialSearch as string);
      setFilter("Tất cả");
    }
  }, [initialSearch]);

  useFocusEffect(useCallback(() => {
    // Đọc chung kho dữ liệu với Staff
  ...
  const handleUpdateStatus = (ticket: any, newStatus: string) => {
    Alert.alert("Quyết định của Admin", `Bạn muốn chuyển Ticket #${ticket.id} sang "${STATUS_META[newStatus]?.label}"?`, [
      { text: "Hủy", style: "cancel" },
      { text: "Xác nhận", style: newStatus === "rejected" ? "destructive" : "default", onPress: async () => {
        const nowStr = new Date().toLocaleString("vi-VN");
        const actionNote = { time: nowStr, text: `[ADMIN] Đã đổi trạng thái thành: ${STATUS_META[newStatus]?.label}` };
        
        const updated = complaints.map(c => c.id === ticket.id ? { 
            ...c, 
            status: newStatus, 
            adminNotes: [actionNote, ...(c.adminNotes || [])] 
        } : c);
        
        await persist(updated);
        setDetail({ ...detail, status: newStatus, adminNotes: [actionNote, ...(detail.adminNotes || [])] });
      }}
    ]);
  };
  ...
  const handleSaveNote = async () => {
    ...
    const newNoteObj = { time: new Date().toLocaleString("vi-VN"), text: `[ADMIN] ${noteInput.trim()}` };
  ...
  const filtered = complaints.filter(c => {
    ...
    if (filter === "Cần Admin can thiệp") return c.status === "escalated";
    if (filter === "Staff đang xử lý") return c.status === "investigating" || c.status === "pending";
  ...
  return (
    <View style={s.container}>
      ...
      <View style={[s.topBar, { paddingTop: insets.top + 10 }]}>
        ...
        <View style={{flex: 1}}>
          <Text style={s.headerTitle}>Quản trị Khiếu nại</Text>
          <Text style={s.headerSub}>Theo dõi & Cấp phép giải quyết</Text>
        </View>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow} style={{ flexGrow: 0, minHeight: 60 }}>
        {FILTERS.map(f => (
          <TouchableOpacity key={f} style={[s.filterChip, filter === f && s.filterActive, f === "Cần Admin can thiệp" && filter !== f && {borderColor: '#fecaca', backgroundColor: '#fff5f5'}]} onPress={() => setFilter(f)}>
            <Text style={[s.filterTxt, filter === f && s.filterTxtActive, f === "Cần Admin can thiệp" && filter !== f && {color: '#ef4444'}]}>
              {f} {f === "Cần Admin can thiệp" && `(${complaints.filter(c => c.status === 'escalated').length})`}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
        {filtered.length === 0 ? (
          <View style={{alignItems: 'center', marginTop: 50}}>
             ...
             <Text style={{color: '#94a3b8', marginTop: 10}}>Chưa có khiếu nại nào ở mục này.</Text>
          </View>
        ) : (
          filtered.map(c => {
            const meta = STATUS_META[c.status] || STATUS_META.pending;
            return (
              <TouchableOpacity key={c.id} style={[s.card, c.status === 'escalated' && {borderColor: '#fecaca', borderWidth: 2}]} activeOpacity={0.8} onPress={() => { setDetail(c); setNoteInput(""); setShowDetail(true); }}>
              ...
              <ScrollView contentContainerStyle={s.modalBody} showsVerticalScrollIndicator={false}>
                ...
                {detail.bookingId && (
                  <TouchableOpacity 
                    ...
                    <Text style={[s.actionBtnTxt, { color: "#2856d6", marginLeft: 8 }]}>Xem thông tin Đơn đặt Tour (#{detail.bookingId})</Text>
                  </TouchableOpacity>
                )}
                ...
                <View style={s.actionSection}>
                  <Text style={s.actionTitle}>Nhật ký xử lý (Staff & Admin)</Text>
                  <View style={s.noteInputBox}>
                    <TextInput style={s.noteInput} placeholder="Admin ghi chú chỉ đạo..." value={noteInput} onChangeText={setNoteInput} multiline />
                    <TouchableOpacity style={s.sendNoteBtn} onPress={handleSaveNote}><Ionicons name="send" size={16} color="#fff" /></TouchableOpacity>
                  </View>
                  
                  {detail.adminNotes?.length === 0 && <Text style={{color: '#94a3b8', fontStyle: 'italic', fontSize: 12}}>Chưa có lịch sử xử lý.</Text>}
                  {detail.adminNotes?.map((note: any, idx: number) => {
                    const isAdmin = note.text.startsWith("[ADMIN]");
                    return (
                      <View key={idx} style={[s.noteItem, isAdmin && {borderColor: '#fca5a5', backgroundColor: '#fef2f2'}]}>
                         <Text style={s.noteTime}>{note.time}</Text>
                         <Text style={[s.noteText, isAdmin && {color: '#dc2626', fontWeight: '600'}]}>{note.text}</Text>
                      </View>
                    );
                  })}
                </View>
                ...
                <View style={s.actionSection}>
                  <Text style={s.actionTitle}>Phán quyết của Admin</Text>
                  <View style={s.actionBtns}>
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#fef9c3", borderWidth: 1, borderColor: "#fde047" }]} onPress={() => handleUpdateStatus(detail, "investigating")}>
                      <Text style={[s.actionBtnTxt, {color: '#d97706'}]}>Trả về Staff điều tra</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#16a34a" }]} onPress={() => handleUpdateStatus(detail, "resolved")}>
                      <Text style={s.actionBtnTxt}>Duyệt: Đã giải quyết</Text>
                    </TouchableOpacity>
                  </View>
                  <TouchableOpacity style={[s.actionBtn, { backgroundColor: "#f3f7ff", marginTop: 8 }]} onPress={() => handleUpdateStatus(detail, "rejected")}>
                    <Text style={[s.actionBtnTxt, { color: "#7a8cc2" }]}>Bác bỏ & Đóng Ticket</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            </View>
      ...
      <AdminTabBar activeRoute="admin-complaints" />
    </View>
  );
}
