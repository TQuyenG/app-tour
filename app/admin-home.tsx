...
const RECENT_ACTIVITIES = [
  { icon: 'checkmark-circle', color: '#22c55e', text: 'Tour Phú Quốc 4N3Đ đã được duyệt', time: '1 giờ trước' },
  { icon: 'person-add', color: '#4f7cff', text: 'HDV mới Lê Minh Tuấn đăng ký', time: '3 giờ trước' },
  { icon: 'ticket', color: '#f59e0b', text: 'Voucher SUMMER35 đạt 28 lượt dùng', time: '5 giờ trước' },
  { icon: 'star', color: '#a855f7', text: 'Đánh giá 5⭐ mới từ khách hàng', time: 'Hôm qua' },
];
...
export default function AdminHome() {
  ...
  const stats = { tours: 12, guides: 8, bookings: 34, vouchers: 5, revenue: '1.45 tỷ', rating: 4.8 };
      ...
      <ScrollView style={styles.main} contentContainerStyle={styles.mainContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroBanner}>
          <View style={styles.heroTextCol}>
            ...
            <Text style={styles.heroTitle}>Doanh thu tháng này{'\n'}đạt 1.45 tỷ đồng!</Text>
            <TouchableOpacity style={styles.heroBtn} onPress={() => router.push('/admin-report' as any)}>
              <Text style={styles.heroBtnText}>Xem báo cáo</Text>
              <Ionicons name="arrow-forward" size={14} color="#4f7cff" />
            </TouchableOpacity>
          </View>
          <MaterialCommunityIcons name="chart-line" size={80} color="rgba(255,255,255,0.18)" />
        </View>
        ...
        <View style={styles.statsGrid}>
          {[
            { icon: 'map-outline', label: 'Tour', value: stats.tours, color: '#4f7cff', bg: '#eef2ff', route: '/admin-tour-management' },
            { icon: 'people-outline', label: 'HDV', value: stats.guides, color: '#22c55e', bg: '#f0fdf4', route: '/admin-guide-management' },
            { icon: 'receipt-outline', label: 'Booking', value: stats.bookings, color: '#f59e0b', bg: '#fffbeb', route: '/admin-report' },
            { icon: 'ticket-outline', label: 'Voucher', value: stats.vouchers, color: '#a855f7', bg: '#fdf4ff', route: '/admin-voucher-management' },
          ].map((s, i) => (
            <TouchableOpacity key={i} style={[styles.statCard, { backgroundColor: s.bg }]} onPress={() => router.push(s.route as any)}>
        ...
        <View style={styles.infoCards}>
          <View style={styles.infoCard}>
            <View style={styles.infoIconWrap}>
              <MaterialCommunityIcons name="cash-multiple" size={20} color="#4f7cff" />
            </View>
            <View>
        ...
        <View style={styles.quickGrid}>
          {[
            ...
            { icon: 'person-add-outline', label: 'Thêm HDV', color: '#22c55e', bg: '#f0fdf4', route: '/admin-guide-management' },
            { icon: 'clipboard-outline', label: 'Duyệt HDV', color: '#d97706', bg: '#fffbeb', route: '/admin-guide-requests' },
            { icon: 'ticket-outline', label: 'Tạo Voucher', color: '#f59e0b', bg: '#fffbeb', route: '/admin-voucher-management' },
            { icon: 'download-outline', label: 'Xuất báo cáo', color: '#a855f7', bg: '#fdf4ff', route: '/admin-report' },
          ].map((q, i) => (
            <TouchableOpacity key={i} style={[styles.quickCard, { backgroundColor: q.bg }]} onPress={() => router.push(q.route as any)}>
        ...
        {BOTTOM_TABS.map((tab, i) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => { setActiveTab(i); router.push(tab.route as any); }}
              activeOpacity={0.75}>
        ...
        <ScrollView style={styles.drawerNav} showsVerticalScrollIndicator={false}>
          {DRAWER_ITEMS.map((item, i) => (
            <TouchableOpacity
              ...
              onPress={() => { closeDrawer(); router.push(item.route as any); }}
              activeOpacity={0.75}>
              ...
              <Text style={[styles.drawerItemLabel, i === 0 && styles.drawerItemLabelActive]}>
                {item.label}
              </Text>
...
const styles = StyleSheet.create({
  ...
  menuBtn: {
    ...
    shadowColor: '#4f7cff', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 3,
  },
  headerCenter: { flex: 1 },
  greeting: { color: '#7a8cc2', fontSize: 12, fontWeight: '500' },
  headTitle: { color: '#1f2a58', fontSize: 20, fontWeight: '800', marginTop: 1 },
  notifBtn: {
    width: 44, height: 44, backgroundColor: '#fff', borderRadius: 14,
    alignItems: 'center', justifyContent: 'center',
    shadowColor: '#4f7cff', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 3,
  ...
  // Hero
  heroBanner: {
    backgroundColor: '#4f7cff', borderRadius: 24, padding: 24,
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    marginBottom: 28,
    shadowColor: '#4f7cff', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.35, shadowRadius: 20, elevation: 8,
    overflow: 'hidden',
  },
  heroTextCol: { flex: 1 },
  ...
  // Section
  sectionTitle: { color: '#1f2a58', fontWeight: '700', fontSize: 15, marginBottom: 12 },
  ...
  // Activity
  activityCard: { backgroundColor: '#fff', borderRadius: 20, paddingVertical: 4, paddingHorizontal: 4, shadowColor: '#4f7cff', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.07, shadowRadius: 10, elevation: 2, marginBottom: 8 },
  ...
  tabBar: {
    ...
    shadowColor: '#1f2a58', shadowOffset: { width: 0, height: -6 }, shadowOpacity: 0.1, shadowRadius: 20, elevation: 16,
  },
  ...
  drawer: {
    ...
    width: DRAWER_WIDTH, backgroundColor: '#1f2a58',
    zIndex: 20,
    borderTopRightRadius: 32, borderBottomRightRadius: 32,
    shadowColor: '#000', shadowOffset: { width: 8, height: 0 }, shadowOpacity: 0.3, shadowRadius: 24, elevation: 24,
    paddingBottom: 32,
  },
  drawerHeader: {
  ...
  drawerAvatar: {
    width: 68, height: 68, borderRadius: 34, backgroundColor: '#4f7cff',
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 2.5, borderColor: 'rgba(255,255,255,0.3)', marginBottom: 12,
  },
