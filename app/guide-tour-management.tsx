...
const SEED: GuideTour[] = [
  {
    id: '1', name: 'Tour Đà Lạt mộng mơ 3N2Đ', category: 'Cao nguyên',
    date: '22/03/2025', duration: '3 ngày 2 đêm', meetingPoint: 'Bến xe Miền Đông, TP.HCM',
    guests: 8, maxGuests: 12, price: '2800000', status: 'assigned',
    customerNames: 'Lê Văn C, Nguyễn Thị E và 6 người khác',
    itinerary: 'Ngày 1: Xuất phát TP.HCM – Đà Lạt, tham quan Hồ Xuân Hương.\nNgày 2: Thác Datanla, Làng hoa Vạn Thành, chợ đêm.\nNgày 3: Đồi chè Cầu Đất, trở về TP.HCM.',
    note: 'Khách có 2 trẻ em, cần chú ý an toàn tại thác nước.',
    assignedBy: 'admin',
  },
  {
    id: '2', name: 'Trekking Langbiang 1 ngày', category: 'Núi rừng',
    date: '25/03/2025', duration: '1 ngày', meetingPoint: 'Cổng Langbiang, Lạc Dương',
    guests: 5, maxGuests: 10, price: '1500000', status: 'assigned',
    customerNames: 'Hoàng Thị F và 4 người',
    itinerary: 'Sáng: Xuất phát leo núi Langbiang – ngắm cảnh.\nTrưa: Picnic trên đỉnh.\nChiều: Xuống núi, trả khách.',
    note: '', assignedBy: 'system',
  },
  {
    id: '3', name: 'Tour Núi Bà Đen khám phá', category: 'Tâm linh',
    date: '15/03/2025', duration: '1 ngày', meetingPoint: 'Khu du lịch Núi Bà Đen, Tây Ninh',
    guests: 4, maxGuests: 4, price: '1200000', status: 'done',
  ...
  {
    id: '4', name: 'Tour Mũi Né biển xanh 2N1Đ', category: 'Biển đảo',
    date: '10/03/2025', duration: '2 ngày 1 đêm', meetingPoint: 'Ga Phan Thiết',
    guests: 6, maxGuests: 15, price: '3600000', status: 'done',
    customerNames: 'Nguyễn Thị D và 5 người',
    itinerary: 'Ngày 1: Đồi cát đỏ, Làng chài Mũi Né, Suối Tiên.\nNgày 2: Đồi cát trắng, tắm biển, trở về.',
    note: '', assignedBy: 'admin',
  },
  {
    id: '5', name: 'Hành trình Hội An cổ kính', category: 'Di sản',
    date: '01/04/2025', duration: '2 ngày 1 đêm', meetingPoint: 'Sân bay Đà Nẵng',
    guests: 0, maxGuests: 10, price: '2200000', status: 'registered',
    customerNames: '',
    itinerary: 'Ngày 1: Phố cổ Hội An, Chùa Cầu, thả đèn hoa đăng.\nNgày 2: Làng gốm Thanh Hà, về Đà Nẵng.',
...
const CATEGORIES = ['Tất cả', 'Biển đảo', 'Cao nguyên', 'Di sản', 'Núi rừng', 'Tâm linh', 'Thành phố'];
const FILTER_STATUS = [
...
export default function GuideTourManagement() {
  ...
  const [regForm, setRegForm] = useState({ name: '', date: '', duration: '', meetingPoint: '', category: 'Biển đảo', maxGuests: '10', note: '' });
  ...
  const updateStatus = (id: string, status: TourStatus) => {
    ...
    Alert.alert('Xác nhận', `Chuyển "${t.name}" sang "${STATUS_MAP[status].label}"?`, [
      { text: 'Hủy', style: 'cancel' },
      { text: 'Đồng ý', onPress: () => persist(tours.map(x => x.id === id ? { ...x, status } : x)) },
    ]);
  };

  const cancelTour = (id: string) => {
    const t = tours.find(x => x.id === id); if (!t) return;
    Alert.alert('Hủy tour', `Bạn muốn hủy tham gia "${t.name}"?\nHành động này sẽ thông báo cho quản trị viên.`, [
      { text: 'Không', style: 'cancel' },
      { text: 'Hủy tour', style: 'destructive', onPress: () => persist(tours.map(x => x.id === id ? { ...x, status: 'cancelled' } : x)) },
    ]);
  };

  const handleRegisterTour = () => {
    if (!regForm.name.trim() || !regForm.date.trim()) {
      Alert.alert('Thiếu thông tin', 'Vui lòng điền tên tour và ngày.'); return;
    }
    const newTour: GuideTour = {
      id: Date.now().toString(),
      name: regForm.name, category: regForm.category, date: regForm.date,
      duration: regForm.duration, meetingPoint: regForm.meetingPoint,
      guests: 0, maxGuests: Number(regForm.maxGuests) || 10,
      price: '0', status: 'registered',
      customerNames: '', itinerary: '', note: regForm.note, assignedBy: 'self',
    };
    persist([...tours, newTour]);
    setRegisterModal(false);
    setRegForm({ name: '', date: '', duration: '', meetingPoint: '', category: 'Biển đảo', maxGuests: '10', note: '' });
    Alert.alert('Đã gửi đăng ký', 'Yêu cầu đăng ký tour đã được gửi. Chờ admin xác nhận.');
  };
  ...
  const rf = (k: keyof typeof regForm, v: string) => setRegForm(p => ({ ...p, [k]: v }));
  ...
  const totalEarned = tours.filter(t => t.status === 'done').reduce((s, t) => s + Number(t.price), 0);
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.topBar}>
        ...
        <TouchableOpacity style={[s.iconBtn, { backgroundColor: '#2856d6' }]} onPress={() => setRegisterModal(true)}>
      ...
      <View style={s.statsRow}>
        {[
          { l: 'Tổng',       v: tours.filter(t => t.status !== 'cancelled').length, c: '#1a2f7a' },
          { l: 'Được giao',  v: assignedTours,  c: '#2856d6' },
          { l: 'Hoàn thành', v: doneTours,      c: '#16a34a' },
          { l: 'Thu nhập',   v: `${(totalEarned / 1000000).toFixed(1)}tr`, c: '#f59e0b' },
        ].map((item, i, arr) => (
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filterRow}>
        {FILTER_STATUS.map(f => (
          <TouchableOpacity key={f.key} style={[s.filterChip, filterStatus === f.key && s.filterActive]} onPress={() => setFilterStatus(f.key)}>
      ...
      <ScrollView contentContainerStyle={s.list}>
        {filtered.length === 0 && (
          <View style={s.emptyWrap}>
            ...
            <TouchableOpacity style={s.emptyBtn} onPress={() => setRegisterModal(true)}>
        ...
        {filtered.map(tour => {
          ...
          const fillPct = tour.maxGuests > 0 ? Math.round((tour.guests / tour.maxGuests) * 100) : 0;

          return (
            <TouchableOpacity key={tour.id} style={s.card} activeOpacity={0.88}
              ...
              <View style={s.cardHeader}>
                <View style={s.cardHeaderLeft}>
                  ...
                  <View style={[s.assignedBadge]}>
                    <Text style={[s.assignedTxt, { color: ab.color }]}>{ab.label}</Text>
                  </View>
                </View>
              ...
              <View style={s.metaGrid}>
                ...
                <View style={s.metaItem}>
                  ...
                  <Text style={s.metaTxt} numberOfLines={1}>{tour.meetingPoint}</Text>
                </View>
              </View>
              ...
              <View style={s.guestsRow}>
                ...
                <Text style={s.guestsTxt}>{tour.guests}/{tour.maxGuests} khách</Text>
              </View>

              {/* Price */}
              <View style={s.priceRow}>
                <Text style={s.price}>{Number(tour.price) > 0 ? `${Number(tour.price).toLocaleString('vi-VN')}đ/người` : 'Chưa có giá'}</Text>
                <Ionicons name={isOpen ? 'chevron-up' : 'chevron-down'} size={16} color="#c0cbe8" />
              </View>
                ...
                <View style={s.expandSection}>
                  ...
                  <View style={s.actionRow}>
                    {tour.status === 'assigned' && (
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#f3e8ff' }]} onPress={() => updateStatus(tour.id, 'ongoing')}>
                        <Ionicons name="play-circle-outline" size={14} color="#a855f7" />
                        <Text style={[s.actionTxt, { color: '#a855f7' }]}>Bắt đầu dẫn</Text>
                      </TouchableOpacity>
                    )}
                    {tour.status === 'ongoing' && (
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#dcfce7' }]} onPress={() => updateStatus(tour.id, 'done')}>
                        <Ionicons name="flag-outline" size={14} color="#16a34a" />
                        <Text style={[s.actionTxt, { color: '#16a34a' }]}>Hoàn thành</Text>
                      </TouchableOpacity>
                    )}
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#eaf0ff' }]} onPress={() => { setNoteText(tour.note); setNoteModal(tour); }}>
                      <Ionicons name="create-outline" size={14} color="#2856d6" />
                      <Text style={[s.actionTxt, { color: '#2856d6' }]}>Ghi chú</Text>
                    </TouchableOpacity>
                    {(tour.status === 'assigned' || tour.status === 'registered') && (
                      <TouchableOpacity style={[s.actionBtn, { backgroundColor: '#fee2e2' }]} onPress={() => cancelTour(tour.id)}>
                        <Ionicons name="close-circle-outline" size={14} color="#dc2626" />
                        <Text style={[s.actionTxt, { color: '#dc2626' }]}>Hủy tham gia</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
          ...
          <View style={s.sheet}>
            ...
            <View style={s.mbody}>
              <TextInput
                ...
                multiline numberOfLines={4} placeholderTextColor="#b0bdd8"
                textAlignVertical="top" autoFocus
              />
              <TouchableOpacity style={s.saveBtn} onPress={() => noteModal && saveNote(noteModal.id, noteText)}>
      ...
      {/* ── Register Tour Modal ── */}
      <Modal visible={registerModal} animationType="slide" transparent onRequestClose={() => setRegisterModal(false)}>
        <KeyboardAvoidingView style={s.overlay} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <TouchableOpacity style={s.backdrop} activeOpacity={1} onPress={() => setRegisterModal(false)} />
          <View style={s.sheet}>
            <View style={s.handle} />
            <View style={s.mheader}>
              <Text style={s.mtitle}>Đăng ký nhận tour</Text>
              <TouchableOpacity onPress={() => setRegisterModal(false)} style={s.closeBtn}>
                <Ionicons name="close" size={20} color="#7a8cc2" />
              </TouchableOpacity>
            </View>
            <ScrollView contentContainerStyle={s.mbody} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              <Text style={s.regHint}>
                💡 Điền thông tin tour bạn muốn nhận dẫn. Admin sẽ xem xét và xác nhận.
              </Text>

              <FL t="Tên tour *" />
              <TextInput style={s.input} value={regForm.name} onChangeText={v => rf('name', v)} placeholder="VD: Tour Phú Quốc 3N2Đ" placeholderTextColor="#b0bdd8" />

              <FL t="Ngày khởi hành *" />
              <TextInput style={s.input} value={regForm.date} onChangeText={v => rf('date', v)} placeholder="VD: 05/04/2025" placeholderTextColor="#b0bdd8" />

              <FL t="Thời gian" />
              <TextInput style={s.input} value={regForm.duration} onChangeText={v => rf('duration', v)} placeholder="VD: 3 ngày 2 đêm" placeholderTextColor="#b0bdd8" />

              <FL t="Điểm tập hợp" />
              <TextInput style={s.input} value={regForm.meetingPoint} onChangeText={v => rf('meetingPoint', v)} placeholder="VD: Sân bay Tân Sơn Nhất" placeholderTextColor="#b0bdd8" />

              <FL t="Danh mục" />
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.optionRow}>
                {['Biển đảo', 'Cao nguyên', 'Di sản', 'Núi rừng', 'Tâm linh', 'Thành phố'].map(c => (
                  <TouchableOpacity key={c} style={[s.optionChip, regForm.category === c && s.optionActive]} onPress={() => rf('category', c)}>
                    <Text style={[s.optionTxt, regForm.category === c && s.optionTxtActive]}>{c}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>

              <FL t="Số khách tối đa" />
              <TextInput style={s.input} value={regForm.maxGuests} onChangeText={v => rf('maxGuests', v)} keyboardType="numeric" placeholderTextColor="#b0bdd8" />

              <FL t="Ghi chú thêm" />
              <TextInput
                style={[s.input, { minHeight: 70, paddingTop: 12 }]}
                value={regForm.note} onChangeText={v => rf('note', v)}
                placeholder="Lý do đăng ký, kỹ năng phù hợp..."
                multiline placeholderTextColor="#b0bdd8" textAlignVertical="top"
              />
...
const s = StyleSheet.create({
  ...
  catChip: { borderRadius: 999, borderWidth: 1, borderColor: '#dfe7ff', backgroundColor: '#fff', paddingHorizontal: 12, paddingVertical: 6 },
  ...
  emptyBtn: { marginTop: 8, backgroundColor: '#2856d6', borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12 },
  emptyBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 14 },

  card: {
    backgroundColor: '#fff', borderRadius: 18, borderWidth: 1, borderColor: '#e4ebff',
    padding: 14, marginBottom: 12,
    shadowColor: '#4f7cff', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.06, shadowRadius: 10, elevation: 2,
  },
  ...
  assignedBadge: { borderRadius: 8, backgroundColor: '#f0f4ff', paddingHorizontal: 8, paddingVertical: 3 },
  assignedTxt: { fontSize: 11, fontWeight: '600' },
  statusBadge: { borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  statusTxt: { fontSize: 11, fontWeight: '700' },

  tourName: { color: '#1a2f7a', fontWeight: '800', fontSize: 15, marginBottom: 10, lineHeight: 22 },

  metaGrid: { gap: 5, marginBottom: 10 },
  metaItem: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  metaTxt: { color: '#7a8cc2', fontSize: 12, flex: 1 },

  guestsRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  ...
  infoBlock: { backgroundColor: '#f7f9ff', borderRadius: 12, padding: 12, marginBottom: 8 },
  infoBlockLabel: { color: '#7a8cc2', fontSize: 11, fontWeight: '700', marginBottom: 4 },
  ...
  regHint: { backgroundColor: '#eaf0ff', borderRadius: 12, padding: 12, color: '#2856d6', fontSize: 12, lineHeight: 18, marginTop: 8, marginBottom: 4 },
  ...
  optionChip: { borderRadius: 999, borderWidth: 1, borderColor: '#dfe7ff', backgroundColor: '#fff', paddingHorizontal: 14, paddingVertical: 7 },
